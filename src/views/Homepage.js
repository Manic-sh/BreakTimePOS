...
class Homepage extends React.Component {
  ...
  handleSubmit = (event) => {
    ...
    axiosInstance.post(KOT_ENDPOINT, data).then((resp) => {
      if (resp.data) {
        const kot = resp.data.id;
        const newOrder = {
          title: `ORDER ${resp.data.id}`,
          kot: resp.data.id,
          active: true,
        };
        axiosInstance.post(ORDERS_ENDPOINT, newOrder).then((response) => {
          const new_order_id = response.data.id;
          const new_url = `/order/${new_order_id}/`;
          this.props.history.push(new_url, { state: kot });
        });
      }
    });
  };
